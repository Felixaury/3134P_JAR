...
motor leftMotorC = motor(PORT14,  ratio6_1, true);    // 13
...
switch(current_auton_selection){
      ...
      //START THE AUTON HERE
      
      // If you need to add any other auton, should be added here, and dont forget modify the if condition on PRE AUTON to reset current_auton_selection value
...
void usercontrol(void) {
  ...
  Controller.ButtonDown.pressed([](){armTargetPosition = 0;});
  Controller.ButtonLeft.pressed([](){armTargetPosition = 14;});
  Controller.ButtonUp.pressed([](){armTargetPosition = 140;});
...
int main() {
  ...
  Competition.autonomous(PositiveCornerRed);
